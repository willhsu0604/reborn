...
class ProgramConsumerSpec extends Specification {

  "ProgramConsumer" should {
    ...
    "run with TestClass main method, then return ok and executed result" in {
      ...
      val pj = ProgramJobConsumer.execute(new ProgramJob("other", pbs, false))
    ...
    "run with FirstTest class, then return true and test result" in {
      ...
      val pj = ProgramJobConsumer.execute(new ProgramJob("firstTest", pbs, true))
      pj.programStatus.exitCode must equalTo(0)
    ...
    "run with SecondTest class, then return false and test error log" in {
      ...
      val pbs = List[ProgramBean](pb1, pb2)
      val pj = ProgramJobConsumer.execute(new ProgramJob("secondTest", pbs, true))
